...
## 阶段四（进阶，$0）：加一条 DMARC 记录

**为什么值得做**：

- Gmail 和 Yahoo 从 2024 年起，对批量发件人**要求具备 DMARC**
- 它是送达率的正向信号（补上后你的认证体系才完整：SPF + DKIM + DMARC）
- 它是将来让品牌 logo 显示在收件箱（BIMI）的**前置条件之一**

**怎么做**（约 1 分钟）：

1. Cloudflare → **mysticdo.com** → **DNS** → **Records** → **Add record**
2. 填写：
   - **Type**：`TXT`
   - **Name**：`_dmarc`
   - **Content**：`v=DMARC1; p=none; rua=mailto:你的邮箱@gmail.com`
     （邮箱换成你收报告用的地址）
3. 保存

**说明**：`p=none` 是**监控模式**——只收集报告，**不影响任何邮件的投递**，是零风险的第一步。等几周后从报告里确认没有异常来源，再考虑升级为 `p=quarantine`（更严格，对声誉更好）。

> 验证：终端执行 `nslookup -type=TXT _dmarc.mysticdo.com`，能返回上面那串内容即生效。

---

## 附：关于收件箱里的「发件人头像」（BIMI）

你可能注意到，读者在 Gmail 里看到的是**一个灰底圆形 + 字母 "M"**——这是默认行为，不是配置错误，绝大多数品牌（除大公司外）都是这样。

要让品牌 logo 出现在那个位置，需要 **BIMI**，门槛如下：

| 覆盖范围 | 需要什么 | 年费 |
|---|---|---|
| Yahoo / AOL | DMARC 强制 + 方形 SVG logo | **免费** |
| Gmail（显示 logo，无蓝色勾） | 再加 **CMC 证书**（要求 logo 在域名上**公开使用满 12 个月**） | 约 $400–1,100 |
| Gmail（logo + 蓝色认证勾）+ Apple Mail | 再加 **VMC 证书**（要求**注册商标**） | 约 $750–1,750 + 商标费 |

共同前置：**DMARC 必须处于 p=quarantine 或 p=reject**（所以上一步是必经之路），且 logo 要做成 SVG Tiny PS 格式（方形、**纯色底不能透明**、无文字——你现在这个透明背景的 mark 需要另做一版）。

**结论**：这件事至少等 12 个月再考虑（CMC 有"公开使用满 12 个月"的硬要求，你的站刚上线）。**现在唯一值得做的是阶段四那条 DMARC。**
