...
在 MailerLite 的代码区搜索 `[REPLACE WITH YOUR REAL POSTAL ADDRESS]`，替换成你的真实邮寄地址。

> ⚠️ **为什么模板里不直接填好？** 因为这个 GitHub 仓库是**公开的** —— 地址一旦写进文件就会随代码公开，并**永久留在提交历史**里（之后删掉也没用）。所以真实地址只在 MailerLite 后台填，不进版本库。

#### 如果地址显示成蓝色链接

这是 **Gmail / iOS Mail 的"自动识别"行为**（把地址变成可点击的地图链接），不是你的代码问题。模板里已经内置了两道防线：

1. **反自动识别的 CSS** —— 用 `color: inherit !important` 让被识别的文本回到页脚原本的灰褐色（`*.x-gmail-data-detectors` / `u + #body a` 等社区验证过的选择器）
2. **零宽字符兜底** —— 如果 CSS 被客户端忽略（有些客户端会剥离 `<style>`），在地址文字里插入 `&zwnj;` 来打断识别：

   ```
   202&zwnj; Zhenshan&zwnj; Rd,&zwnj; Suzhou,&zwnj; China
   ```

   `&zwnj;`（零宽非连接符）在页面和收件箱里**完全不可见**，但会让客户端的"地址正则"匹配失败，于是它就不会加链接、也就不会染成蓝色。

> 💡 顺序建议：先试第 1 道（重新粘贴模板即可）。如果地址还是蓝的，再手动加上 `&zwnj;`——那一招几乎必定生效。
